...
class TradingEnv(gym.Env):
    ...
    Index must be DatetimeIndex. Your desired inputs need to contain 'feature' in their
    column name : this way, they will be returned as observation at each step.
    :type df: pandas.DataFrame

    :param price_column_name: The name of the column which will be used as a price
    to calculate PnL. The default is 'close'.
    :type price_column_name: str
    ...
    def __init__(self,
        ...
        self._set_df(df, price_column_name)
    ...
    def _set_df(self, df, price_column_name):
        df = df.copy()
        self._features_columns = [col for col in df.columns if "feature" in col]
        # a set of column names (plus 'close') that are not feature columns.
        ...
        self._price_array = np.array(self.df[price_column_name])
