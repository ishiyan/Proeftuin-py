...
class History:
    """
    This class is a simple DataFrame-like storage for the history of the environment.
    It stores training information at each timestep of the training.
    You can use it in three different ways:
    
    - history['column name', t]
      Get a scalar value of the column ‘column name’ at time step t.
    - history['column name']
      Get a numpy array with all the values of the column ‘column name’ from timestep 0 to current timestep.
    - history[t]
      Get a dictionary with all keys with the associated values at time step t.

    It gathers info from your initial DataFrame and labels them with data_{column_name}
    like : data_close , data_open , data_high ….
    ...
    def add(self, **kwargs):
        ...
        if columns == self.columns:
            ...
            raise ValueError(f"Make sure your inputs match the initial ones... Initial ones : {self.columns}. New ones {columns}")
