from typing import List, NamedTuple, Type, Union, Optional, Literal
...
class BinanceMonthlyTradesProvider(Provider):
    """
    ...
    monthly trades archives from (binance.com)[binance.com]. Then it converts
    ...
    def __init__(self,
        ...
        while d < date_to:
            # Check if we have '.feather' file for trades
    ...
    def convert_month_archive(file_path_zip: str):
        ...
        df = pd.read_csv(
            ...
            sep=',',
            header=None,
            names=['id', 'price', 'amount', 'quoteQty', 'datetime', 'isSell',
                'isBestMatch'],
            dtype={'id': np.int64, 'price': np.float32, 'amount': np.float32,
                'quoteQty': np.float32, 'datetime': np.int64, 'isSell': bool,
                'isBestMatch': bool},
            index_col=None)
    ...
    def reset(self,
        ...
        if seek is not None:
            ...
            trade_index = self._df.datetime.searchsorted(
                episode_start_datetime, side='right')
            trade_index = min(trade_index, len(self._df) - 1)

        # Get the actual episode start datetime as it may be later
        # than demanded datetime.
        self._episode_start_datetime = \
            self._df.datetime.iloc[trade_index].to_pydatetime()
