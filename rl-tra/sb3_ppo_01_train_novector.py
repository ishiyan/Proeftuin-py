...
evaluate_episodes_every_iteration=5 # Set to 0 to disable
verbose=1
...
def create_env(vec_env_index: int = None, symbol: str = 'SINE',
    ...
    env = Monitor(env,
        filename=f'{eval_dir}eval_{iteration}_{name}{s}' if eval else f'{dir}iter_{iteration}_{name}{s}',
        allow_early_resets=False,
        info_keywords=('vec_env_index','episode_number','step_number','provider_name','aggregator_name'))
    return env
...
def create_envs(symbol: str='SINE', iteration: int=1,
    ...
    return [lambda i=i: create_env(vec_env_index=i, symbol=symbol,
        iteration=iteration, eval=eval) for i in range(num_cores)]
...
if __name__=="__main__":
    for iteration in range(start_iteration_number, total_iterations):
        ...
        if evaluate_episodes_every_iteration > 0:
            ...
            env = SubprocVecEnv(create_envs(symbol='BTCEUR', iteration=iteration, eval=True, max_envs=4))
