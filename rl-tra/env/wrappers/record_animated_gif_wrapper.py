...
class RecordAnimatedGIF(
    ...
    def __init__(
        ...
        if not self.record_episodes and not self.record_steps:
            raise ValueError('At least one of record_episodes_end or ' \
                             'record_episodes_steps must be True.')
    ...
    def flush_episodes(self):
        name = f'{self.name_prefix}-episodes_{self.animation_episode_first}_{self.animation_episode_last}.gif'
