...
class RoundtripPerformance:
    ...
    def average_duration_seconds(self) -> float:
        ...
        return sum([r.duration.total_seconds() for r in self.roundtrips]) / self.total_count()
    ...
    def average_gross_winning_duration_seconds(self) -> float:
        ...
        return sum([r.duration.total_seconds() for r in self.roundtrips if r.gross_pnl > 0]) / self.gross_winning_count()
    ...
    def average_gross_loosing_duration_seconds(self) -> float:
        ...
        return sum([r.duration.total_seconds() for r in self.roundtrips if r.gross_pnl < 0]) / self.gross_loosing_count()
    
    def average_net_winning_duration_seconds(self) -> float:
        """Returns the average duration of a net winning roundtrip in seconds."""
        return sum([r.duration.total_seconds() for r in self.roundtrips if r.net_pnl > 0]) / self.net_winning_count()
    
    def average_net_loosing_duration_seconds(self) -> float:
        """Returns the average duration of a net loosing roundtrip in seconds."""
        return sum([r.duration.total_seconds() for r in self.roundtrips if r.net_pnl < 0]) / self.net_loosing_count()
    ...
    def average_maximum_adverse_excursion (self) -> float:
        ...
        return sum([r.maximum_adverse_excursion for r in self.roundtrips]) / self.total_count()
    ...
    def max_consecutive_net_winners (self) -> int:
        ...
        return max([len(list(g)) for k, g in groupby([r.net_pnl > 0 for r in self.roundtrips]) if k])
